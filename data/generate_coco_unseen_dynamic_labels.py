#!/usr/bin/env python3
"""
Generate per-image label sets for the COCO2017 unseen validation subset.

For each image, we take the annotated unseen categories and extend them with a
random 50% sample of the remaining unseen categories. The sampling is
deterministic per image to ensure reproducibility, and the resulting label
lists are persisted to disk for later reuse.
"""
...
DEFAULT_METADATA = Path("data/processed/coco2017/val/instances_val2017_unseen.json")
DEFAULT_OUTPUT = Path("data/processed/coco2017/val/coco_unseen_dynamic_labels.json")
DEFAULT_CLASSES_OUTPUT = Path("data/processed/coco2017/val/coco_unseen_classes.txt")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build per-image label sets for COCO unseen validation."
    )
    parser.add_argument(
        "--metadata",
        type=Path,
        default=DEFAULT_METADATA,
        help="Path to COCO metadata JSON (unseen split).",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    print(f"Total unseen classes in COCO: {len(all_classes)}")
