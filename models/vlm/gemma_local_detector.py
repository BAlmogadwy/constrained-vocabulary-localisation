...
def _select_model_cls(model_id: str):
    ...
    multimodal_types = {"gemma3", "gemma3_vl", "gemma2", "gemma2_vl"}
    if model_type in multimodal_types:
        ...
        raise RuntimeError(
            "This transformers installation lacks support for Gemma vision models. "
            "Please upgrade transformers (>=4.45) to access AutoModelForImageTextToText."
        )
...
class GemmaLocalDetector:
    """Local Gemma 3 vision detector using transformers."""
    ...
    def __init__(
        ...
        env_model = os.getenv("GEMMA_LOCAL_MODEL", "google/gemma-3-12b-it")
        path_override = os.getenv("GEMMA_LOCAL_PATH")
        if path_override:
            self.model_id = str(Path(path_override).expanduser().resolve())
        else:
            self.model_id = model_id or env_model

        requested_device = forced_device or os.getenv("GEMMA_DEVICE") or (
        ...
        default_tokens = int(os.getenv("GEMMA_LOCAL_MAX_NEW_TOKENS", "512"))
        self.max_new_tokens = max_new_tokens or default_tokens

        print(f"Loading Gemma model {self.model_id} on {self.device} (dtype={self.dtype}).")
        try:
            self.processor = AutoProcessor.from_pretrained(self.model_id)
        except Exception as exc:
            raise RuntimeError(
                "Failed to load Gemma processor. Ensure you executed "
                "'huggingface-cli login --token <HF_TOKEN>' and have been granted access "
                f"to the gated repository: {self.model_id}"
            ) from exc
        ...
        try:
            ...
            raise RuntimeError(
                "Unable to load Gemma model weights. Confirm the checkpoint is available locally "
                "or that your Hugging Face token is configured with gated-repo access."
            ) from exc
    ...
    def _generate(self, prompt: str, image: Image.Image, tokens: int) -> str:
        ...
        if hasattr(self.processor, "apply_chat_template"):
            try:
                ...
                print(f"⚠️ Gemma chat template fallback triggered: {exc}")
                inputs = None
    ...
    def _parse_detections(raw_text: str) -> List[Dict[str, Any]]:
        if not raw_text:
            return []
        cleaned = GemmaLocalDetector._strip_markup(raw_text)
