...
Run from this directory: `python lvis_paired_bootstrap.py --bootstrap 1000 --seed 12345`.
...
Dependency: the task-local `lvis==0.5.3` package in `_deps`, with the removed `np.float` alias restored to `float` in memory for NumPy compatibility; evaluator source and scoring algorithm are unchanged. The script records hashes of the evaluator files and supporting scripts.
